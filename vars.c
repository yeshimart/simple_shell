...
int replace_vars(info_xyz *info)
{
	...
	for (i = 0; info->argv[i]; i++)
	{
		...
		{
			subst_string(&(info->argv[i]),
				dup_strd(con_number(info->status, 10, 0)));
			continue;
		}
		if (!stringCompare(info->argv[i], "$$"))
		{
			subst_string(&(info->argv[i]),
				dup_strd(con_number(getpid(), 10, 0)));
			continue;
		}
		...
		{
			subst_string(&(info->argv[i]),
				dup_strd(findCharacter(node->str, '=') + 1));
			continue;
		}
		subst_string(&info->argv[i], dup_strd(""));
